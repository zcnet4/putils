...
namespace putils
{
    ...
    public:
        struct Client
        {
            ...
            template<typename T>
            T getBuff() const
            { return reinterpret_cast<const T>(*(_buff.get())); }

            std::string getMsg() const
            { return std::string(_buff.get()); }

            template<typename T>
            void send(T&& data)
            { _connection.send(*this, FWD(data)); }
    ...
    private:
        ...
        {
            if (std::find_if(_clients.begin(), _clients.end(), [&client](const auto& c)
            { return c.get() == &client; })
                == _clients.end())
    ...
    private:
        ...
        T receive(Client& client, bool selectAlreadyRunning = false, timeval timeout = {-1, -1})
        {
            ...
            const auto rmid = client.disconnected.addObserver([&done]
                                                              { done = true; });
...
/*            if (_toWrite.find(fd) == _toWrite.end())
        ...
        {
            auto it = std::find_if(_clients.cbegin(), _clients.cend(), [fd](const auto& c)
            { return c->fd == fd; });
            if (it == _clients.end())
        ...
        {
            auto it = std::find_if(_clients.begin(), _clients.end(), [fd](const auto& c)
            { return c->fd == fd; });
            if (it == _clients.end())
                throw std::runtime_error("No such client");
            return **it;
    ...
    template<>
    inline std::string ATCPListener::Client::getBuff() const
    { return getMsg(); }
