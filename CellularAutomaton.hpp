...
namespace putils
{
    ...
    {
    public:
        using NeighborFetcher = std::function<
                std::vector<const Cell<Contained>*>(const std::vector<Cell<Contained>>& cells,
                                                    const Point<int, 3>& pos)>;
        using ModifierCondition = std::function<
                bool(const std::vector<const Cell<Contained>*>& neighbors, const Cell<Contained>& cell)>;
    ...
    public:
        const std::vector<Cell<Contained>>& getCells() const noexcept
        { return _cells; }

        std::vector<Cell<Contained>>& getCells() noexcept
        { return _cells; }
    ...
    template<typename Contained>
    std::vector<const Cell<Contained>*>
    CellularAutomaton<Contained>::getNeighbors(const Point<int, 3>& pos) const noexcept
    {
