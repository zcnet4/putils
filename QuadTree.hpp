...
namespace putils
{
    ...
    public:
        ...
        void add(Obj &&obj, const Point<Precision> &pos, const Point<Precision> &size = {0, 0}) noexcept
        {
            if (_parent == nullptr &&
    ...
    private:
        ...
        {
            return putils::Rect<Precision>{ pos, size }
                    .intersect(
                            putils::Rect<Precision>{ _topLeft, _size }
                    );
...
#include <assert.h>
    namespace test
    {
        ...
        {
            {
                putils::QuadTree<int> tree({0, 0}, {2, 2});
