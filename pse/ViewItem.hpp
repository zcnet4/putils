...
namespace pse
{
    ...
    public:
        const sf::Vector2f& getDestination() const noexcept
        { return _destination; }

        void setDestination(const sf::Vector2f& destination) noexcept
        { _destination = destination; }

        // Previous position getters and setters
    public:
        const sf::Vector2f& getPreviousPosition() const noexcept
        { return _previousPosition; }

        void setPreviousPosition(const sf::Vector2f& pos) noexcept
        { _previousPosition = pos; }

        // Movement getters and setters (generally no need to call these, go straight through goTo
    public:
        void setDelay(double delay) noexcept
        { _moveTimer.setDuration(std::chrono::duration<double>(delay)); }

        void setDelay(const std::chrono::duration<double>& delay) noexcept
        { _moveTimer.setDuration(delay); }

        void startMove() noexcept
        { _moveTimer.restart(); }

        double getMoveDuration() const noexcept
        { return _moveTimer.getDuration().count(); }

        double getMoveTimeLeft() const noexcept
        { return _moveTimer.getTimeLeft().count(); }
